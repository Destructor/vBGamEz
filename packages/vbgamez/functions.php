...
/* ALL INFO IN THE FILE BY LGSL AUTHOR

   edited functions: vbgamez_query_live; vbgamez_query_direct; vbgamez_type_list;

   Other functions renamed and not edited;
...
*/
 ...
 \-----------------------------------------------------------------------------------------------------------*/
  ...
  function vbgamez_type_list($skip_conditions = false)

  {

     $gamearray = array();

    if(!defined('VBG_HIDE_NOTDB_GAMES'))
    ...
    //"callofdutybo" => "Call Of Duty: Black Ops",
    ...
    "source"        => "Source (Half-Life 2, TF2, L4D, ..)",
    ...
    if(!class_exists('vBGamez_dbGames_Bootstrap'))
    ...
    $vb_area_cond = (VB_AREA == 'AdminCP' AND $_REQUEST['do'] == 'options' AND $_REQUEST['dogroup'] == 'vbgamez');



    if(((defined('VBG_HIDE_NOTDB_GAMES') AND VBG_HIDE_NOTDB_GAMES == 1) AND (VB_AREA == 'AdminCP' OR VB_AREA == 'ModCP')) OR (defined('VBG_PACKAGE') AND VBG_PACKAGE == true) OR $vb_area_cond OR defined('VBG_IS_BLOCK'))

    {

    	$db_gametypes = vBGamez_dbGames_Bootstrap::get_db_gameTypes();
...

    	{
              ...
              foreach($db_gametypes AS $gametype => $game)

              {

                      $gamearray[$gametype] = $game['name'];

              }



              // re-sort $gamearray array
	...
	foreach($gamearray AS $type => $name)

	{

		if($newtype = vB_vBGamez::renameGameType($type, null))

		{

			$gamearray[$type] = $newtype;

		}

	}

    if(!$vb_area_cond AND !$skip_conditions)

    {



    		if(!class_exists('vB_vBGamez_Gamelist'))
...

		{
				...
				return array('' => $vbphrase['vbgamez_check_enabled_games']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_protocol_list()

  {

    $vbgamez_protocol_list = array(
    ...
    //if(defined('VBG_HIDE_NOTDB_GAMES') OR (VB_AREA != 'AdminCP' AND VB_AREA !=' ModCP'))
    ...
    //{
...

               {
                       ...
                       foreach(vBGamez_dbGames_Bootstrap::get_db_gameTypes() AS $gametype => $game)

                       {

                                 $vbgamez_protocol_list[$gametype] = $gametype;
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_software_link($type, $ip, $c_port, $q_port, $s_port)

  {

	global $vbphrase;

	if($vbphrase['vbgamez_empty_connection_link'])

	{

		$error_message = "javascript:alert('" . $vbphrase['vbgamez_empty_connection_link'] . "');";
...

		{
			...
			$error_message = $vbulletin->options['bburl'];

		}else{

			$error_message = 'http://'.$_SERVER['SERVER_NAME'];
    ...
    "callofdutybo" => "qtracker://{IP}:{S_PORT}?game=CallOfDutyBlackOps&action=show",
    ...
    "doomskulltag"  => $error_message,

    "doomzdaemon"   => $error_message,

    "doom3"         => "qtracker://{IP}:{S_PORT}?game=Doom3&action=show",

    "dh2005"        => $error_message,
    ...
    "freelancer"    => $error_message,

    "frontlines"    => $error_message,

    "f1c9902"       => $error_message,

    "gamespy1"      => $error_message,

    "gamespy2"      => $error_message,

    "gamespy3"      => $error_message,

    "ghostrecon"    => $error_message,
    ...
    "wow"           =>  vB_vBGamez::fetch_scriptname()."?do=realmlist&ip={IP}&port={S_PORT}",  // ADDED BY VBGAMEZ DEVELOPER

    "wolfrtcw"      => "qtracker://{IP}:{S_PORT}?game=ReturntoCastleWolfenstein&action=show",

    "wolf2009"      => $error_message);
...

    {
...

              {
                      ...
                      $vbgamez_software_link[$gametype] = str_replace('{vbg_script}', vB_vBGamez::fetch_scriptname(), $game['url']);
    ...
    if (!isset($vbgamez_software_link[$type])) { $type = str_replace("_", "", $type); }



    // INSERT DATA INTO STATIC LINK - CONVERT SPECIAL CHARACTERS - RETURN

    return str_replace(array("{IP}", "{C_PORT}", "{Q_PORT}", "{S_PORT}"), array($ip, $c_port, $q_port, $s_port), $vbgamez_software_link[$type]);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_port_conversion($type, $c_port, $q_port, $s_port)
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_live($type, $ip, $c_port, $q_port, $s_port, $request, $dbinfo = '')
...
//---------------------------------------------------------+
...

    {
      ...
      vB_vBGamez::error("vbgamez PROBLEM: INVALID IP OR HOSTNAME");
    ...
    if (!isset($vbgamez_protocol_list[$type]))

    {

      vB_vBGamez::error("vbgamez PROBLEM: ".($type ? "INVALID TYPE '{$type}'" : "MISSING TYPE")." FOR {$ip}, {$c_port}, {$q_port}, {$s_port}");

    }



    $vbgamez_function = "vbgamez_query_{$vbgamez_protocol_list[$type]}";



    if (!function_exists($vbgamez_function))

    {

      vB_vBGamez::error("vbgamez PROBLEM: FUNCTION DOES NOT EXIST FOR: {$type}");
...

    {
      ...
      vB_vBGamez::error("vbgamez PROBLEM: INVALID QUERY PORT");
...
//  GET DATA
    ...
    if ($vbgamez_function == "vbgamez_query_01") // TEST RETURNS DIRECT

    {

      $vbgamez_need = ""; $vbgamez_fp = "";

      $response = call_user_func_array($vbgamez_function, array(&$server, &$vbgamez_need, &$vbgamez_fp));
...

    {
                  ...
                  foreach($db_gametypes AS $gametype => $data)

		  {

			  $exclude_games[] = $gametype;
    ...
    if ($vbulletin->options['vbgamez_feed'] AND !in_array($type, $exclude_games))

    {

      $response = vbgamez_query_feed($server, $request, $vbulletin->options['vbgamez_feed'], $vbulletin->options['vbgamez_feed_url']);

    }

    elseif ($vbgamez_function == "vbgamez_query_30" || $vbgamez_function == "vbgamez_query_97")

    {

      $response = vbgamez_query_direct($server, $request, $vbgamez_function, "tcp");
...

    {
      ...
      $response = vbgamez_query_direct($server, $request, $vbgamez_function, "udp", $dbinfo);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_direct(&$server, $request, $vbgamez_function, $scheme, $dbinfo = '')
...
//---------------------------------------------------------+
    ...
    $vbgamez_fp = @fsockopen("{$scheme}://{$server['b']['ip']}", $server['b']['q_port'], $errno, $errstr, 1);



    if (!$vbgamez_fp) { return FALSE; }
...
//---------------------------------------------------------+
    ...
    $vbulletin->options['vbgamez_timeout'] = intval($vbulletin->options['vbgamez_timeout']);



    stream_set_timeout($vbgamez_fp, $vbulletin->options['vbgamez_timeout'], $vbulletin->options['vbgamez_timeout'] ? 0 : 500000);



    stream_set_blocking($vbgamez_fp, TRUE);
...
//  CHECK WHAT IS NEEDED
    ...
    $vbgamez_need['s'] = strpos($request, "s") !== FALSE ? TRUE : FALSE;

    $vbgamez_need['e'] = strpos($request, "e") !== FALSE ? TRUE : FALSE;

    $vbgamez_need['p'] = strpos($request, "p") !== FALSE ? TRUE : FALSE;



    // ChANGE [e] TO [s][e] AS BASIC QUERIES OFTEN RETURN EXTRA INFO

    if ($vbgamez_need['e'] && !$vbgamez_need['s']) { $vbgamez_need['s'] = TRUE; }
...
//  QUERY FUNCTION IS REPEATED TO REDUCE DUPLICATE CODE
...

    {
      ...
      $vbgamez_need_check = $vbgamez_need;
      ...
      // AVOID WRITING $vbgamez_need = FALSE FALSE FALSE FOR GAMES THAT RETURN ALL DATA IN ONE RESPONSE

      if ($vbgamez_need_check == $vbgamez_need) { break; }



      // OPTIMIZATION THAT SKIPS REQUEST FOR PLAYER DETAILS WHEN THE SERVER IS KNOWN TO BE EMPTY

      if ($vbgamez_need['p'] && $server['s']['players'] == "0") { $vbgamez_need['p'] = FALSE; }

    }

    while ($vbgamez_need['s'] == TRUE || $vbgamez_need['e'] == TRUE || $vbgamez_need['p'] == TRUE);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_01(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//MINECRAFT------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_999(&$server, &$vbgamez_need, &$vbgamez_fp) 

  { 

    // get challenge 

    fwrite($vbgamez_fp, "\xFE\xFD\x09\x01\x02\x03\x04"); 

    $Data         = SubStr(fread($vbgamez_fp, 1440),5); 
    ...
    fwrite($vbgamez_fp, "\xFE\xFD\x00\x01\x02\x03\x04".$Challenge."\x01\x02\x03\x04"); 

    $buffer     = substr(fread($vbgamez_fp, 1440),16); 
...
//MINECRAFT------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_02(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    if     ($server['b']['type'] == "quake2")              { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFstatus");        }

    elseif ($server['b']['type'] == "warsowold")           { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFgetinfo");       }

    elseif (strpos($server['b']['type'], "moh") !== FALSE) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x02getstatus"); } // mohaa_ mohaab_ mohaas_ mohpa_

    else                                                   { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFgetstatus");     }



    $buffer = fread($vbgamez_fp, 4096);
...
//---------------------------------------------------------+
...

    {
      ...
      $data_key               = strtolower(vbgamez_parse_color($data_key, "1"));

      $server['e'][$data_key] = vbgamez_parse_color($item[$item_key+1], "1");
...
//---------------------------------------------------------+
...

    {
      ...
      $server['p'][$player_key]['name'] = vbgamez_parse_color($server['p'][$player_key]['name'], "1");
...

      {
        ...
        $server['p'][$player_key]['time'] = vbgamez_time($server['p'][$player_key]['time']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_03(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    if ($server['b']['type'] == "bf1942" && $vbgamez_need['p'] && !$vbgamez_need['s'] && !isset($vbgamez_need['sp'])) { $vbgamez_need['s'] = TRUE; $vbgamez_need['sp'] = TRUE; }



    if     ($server['b']['type'] == "cncrenegade") { fwrite($vbgamez_fp, "\\status\\"); }

    elseif ($vbgamez_need['s'] || $vbgamez_need['e'])    { fwrite($vbgamez_fp, "\\basic\\\\info\\\\rules\\"); $vbgamez_need['s'] = FALSE; $vbgamez_need['e'] = FALSE; }

    elseif ($vbgamez_need['p'])                       { fwrite($vbgamez_fp, "\\players\\");                $vbgamez_need['p'] = FALSE; }
...
//---------------------------------------------------------+
...

    {
...

      {
        ...
        $key   = strtolower(vbgamez_cut_string($packet, 0, "\\"));

        $value =       trim(vbgamez_cut_string($packet, 0, "\\"));
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_04(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
...

    {
      ...
      $data_key = isset($vbgamez_ravenshield_key[$tmp[0]]) ? $vbgamez_ravenshield_key[$tmp[0]] : $tmp[0]; // CONVERT TO DESCRIPTIVE KEYS
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_05(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://developer.valvesoftware.com/wiki/Server_Queries
...

    {
      ...
      if     ($vbgamez_need['s']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFdetails\x00"); }

      elseif ($vbgamez_need['p']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFplayers\x00"); }

      elseif ($vbgamez_need['e']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFrules\x00");   }
...

    {
      ...
      $challenge_code = isset($vbgamez_need['challenge']) ? $vbgamez_need['challenge'] : "\x00\x00\x00\x00";



      if     ($vbgamez_need['s']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x54Source Engine Query\x00"); }

      elseif ($vbgamez_need['p']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x55{$challenge_code}");       }

      elseif ($vbgamez_need['e']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x56{$challenge_code}");       }
...
//  ONCE WE KNOW THE TYPE WE CAN FIND THE TOTAL NUMBER OF PACKETS EXPECTED
...

    {
      ...
      $packet = fread($vbgamez_fp, 4096); if (!$packet) { return FALSE; }
      ...
      if     ($vbgamez_need['s']) { if ($packet[4] == "D")                                           { continue; } }

      elseif ($vbgamez_need['p']) { if ($packet[4] == "m" || $packet[4] == "I")                      { continue; } }

      elseif ($vbgamez_need['e']) { if ($packet[4] == "m" || $packet[4] == "I" || $packet[4] == "D") { continue; } }

      //---------------------------------------------------------------------------------------------------------------------------------+



      if     (substr($packet, 0,  5) == "\xFF\xFF\xFF\xFF\x41") { $vbgamez_need['challenge'] = substr($packet, 5,  4); return TRUE; } // REPEAT WITH GIVEN CHALLENGE CODE
...
//  THEN REMOVE THE STANDARD HEADER AND CHECK ITS CORRECT
...

    {
...

      {
        ...
        $server['e']['bzip2'] = "unavailable"; $vbgamez_need['e'] = FALSE;
    ...
    $header = vbgamez_cut_byte($buffer, 4);
...
//---------------------------------------------------------+
    ...
    $response_type = vbgamez_cut_byte($buffer, 1);
...

    {
      ...
      $server['e']['netcode']     = ord(vbgamez_cut_byte($buffer, 1));

      $server['s']['name']        = vbgamez_cut_string($buffer);

      $server['s']['map']         = vbgamez_cut_string($buffer);

      $server['s']['game']        = vbgamez_cut_string($buffer);

      $server['e']['description'] = vbgamez_cut_string($buffer);

      $server['e']['appid']       = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

      $server['s']['players']     = ord(vbgamez_cut_byte($buffer, 1));

      $server['s']['playersmax']  = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['bots']        = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['dedicated']   = vbgamez_cut_byte($buffer, 1);

      $server['e']['os']          = vbgamez_cut_byte($buffer, 1);

      $server['s']['password']    = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['anticheat']   = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['version']     = vbgamez_cut_string($buffer);
...

    {
      ...
      $server_ip                  = vbgamez_cut_string($buffer);
      ...
      if (ord(vbgamez_cut_byte($buffer, 1))) // MOD FIELDS ( OFF FOR SOME HALFLIFEWON-VALVE SERVERS )

      {

        $server['e']['mod_url_info']     = vbgamez_cut_string($buffer);

        $server['e']['mod_url_download'] = vbgamez_cut_string($buffer);

        $buffer = substr($buffer, 1);

        $server['e']['mod_version']      = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

        $server['e']['mod_size']         = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

        $server['e']['mod_server_side']  = ord(vbgamez_cut_byte($buffer, 1));

        $server['e']['mod_custom_dll']   = ord(vbgamez_cut_byte($buffer, 1));
...

    {
      ...
      $returned = ord(vbgamez_cut_byte($buffer, 1));
...

      {
        ...
        $server['p'][$player_key]['pid']   = ord(vbgamez_cut_byte($buffer, 1));

        $server['p'][$player_key]['name']  = vbgamez_cut_string($buffer);

        $server['p'][$player_key]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

        $server['p'][$player_key]['time']  = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));
...

    {
      ...
      $returned = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

      {
        ...
        $item_key   = strtolower(vbgamez_cut_string($buffer));

        $item_value = vbgamez_cut_string($buffer);
...
//---------------------------------------------------------+
    ...
    if ($vbgamez_need['s'] && !$vbgamez_need['e']) { $server['e'] = array(); }



    if     ($vbgamez_need['s']) { $vbgamez_need['s'] = FALSE; }

    elseif ($vbgamez_need['p']) { $vbgamez_need['p'] = FALSE; }

    elseif ($vbgamez_need['e']) { $vbgamez_need['e'] = FALSE; }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_06(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  GET A CHALLENGE CODE IF NEEDED
...

    {
      ...
      fwrite($vbgamez_fp, "\xFE\xFD\x09\x21\x21\x21\x21\xFF\xFF\xFF\x01");



      $challenge_packet = fread($vbgamez_fp, 4096);
    ...
    fwrite($vbgamez_fp, "\xFE\xFD\x00\x21\x21\x21\x21{$challenge_code}\xFF\xFF\xFF\x01");
...
//  GET RAW PACKET DATA
...

    {
      ...
      $packet_order = ord(vbgamez_cut_byte($packet, 1));
...
//  SERVER SETTINGS
    ...
    while ($key = strtolower(vbgamez_cut_string($buffer)))

    {

      $server['e'][$key] = vbgamez_cut_string($buffer);

    }



    $vbgamez_conversion = array("name"=>"hostname", "game"=>"gamename", "map"=>"mapname", "players"=>"numplayers", "playersmax"=>"maxplayers", "password"=>"password");

    foreach ($vbgamez_conversion as $s => $e) { if (isset($server['e'][$e])) { $server['s'][$s] = $server['e'][$e]; unset($server['e'][$e]); } } // vbgamez STANDARD



    if ($server['b']['type'] == "bf2" || $server['b']['type'] == "bf2142") { $server['s']['map'] = ucwords(str_replace("_", " ", $server['s']['map'])); } // MAP NAME CONSISTENCY

elseif (strtolower($server['e']['gametype']) == "dayz") { $server['s']['game'] = 'dayz'; }
...
//  PLAYER DETAILS
...

    {
      ...
      $field = vbgamez_cut_string($buffer, 0, "\x00\x00");
...
elseif ($field == 'team' && $server['b']['type'] == 'arma2') {
vbgamez_cut_string($buffer, 0, "score_\x00\x00"); $field = 'score';
if (!$buffer) { break; }
}
      ...
      $value_list = vbgamez_cut_string($buffer, 0, "\x00\x00");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_07(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFstatus\x00");
...
//---------------------------------------------------------+
...

    {
      ...
      $server['p'][$key]['name']        = vbgamez_parse_color($match[5], $server['b']['type']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_08(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "s"); // ASE ( ALL SEEING EYE ) PROTOCOL
...
//---------------------------------------------------------+
    ...
    $server['e']['gamename']   = vbgamez_cut_pascal($buffer, 1, -1);

    $server['e']['hostport']   = vbgamez_cut_pascal($buffer, 1, -1);

    $server['s']['name']       = vbgamez_parse_color(vbgamez_cut_pascal($buffer, 1, -1), $server['b']['type']);

    $server['e']['gamemode']   = vbgamez_cut_pascal($buffer, 1, -1);

    $server['s']['map']        = vbgamez_cut_pascal($buffer, 1, -1);

    $server['e']['version']    = vbgamez_cut_pascal($buffer, 1, -1);

    $server['s']['password']   = vbgamez_cut_pascal($buffer, 1, -1);

    $server['s']['players']    = vbgamez_cut_pascal($buffer, 1, -1);

    $server['s']['playersmax'] = vbgamez_cut_pascal($buffer, 1, -1);
...

    {
      ...
      $item_key   = strtolower(vbgamez_cut_pascal($buffer, 1, -1));

      $item_value = vbgamez_cut_pascal($buffer, 1, -1);
...
//---------------------------------------------------------+
...

    {
      ...
      $bit_flags = vbgamez_cut_byte($buffer, 1); // FIELDS HARD CODED BELOW BECAUSE GAMES DO NOT USE THEM PROPERLY
...

      {
        ...
        if ($item_key == "name") { vbgamez_parse_color($item_value, $server['b']['type']); }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_09(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    if ($server['b']['type'] == "serioussam2") { $vbgamez_need['p'] = FALSE; if (!$vbgamez_need['s'] && !$vbgamez_need['e']) { $vbgamez_need['s'] = TRUE; } }
...
//---------------------------------------------------------+
    ...
    if ($vbgamez_need['s'] || $vbgamez_need['e'])

    {

      $vbgamez_need['s'] = FALSE; $vbgamez_need['e'] = FALSE;



      fwrite($vbgamez_fp, "\xFE\xFD\x00\x21\x21\x21\x21\xFF\x00\x00\x00");
...
//---------------------------------------------------------+
    ...
    elseif ($vbgamez_need['p'])

    {

      $vbgamez_need['p'] = FALSE;



      fwrite($vbgamez_fp, "\xFE\xFD\x00\x21\x21\x21\x21\x00\xFF\x00\x00");
      ...
      $buffer[0]  = str_replace("player", "name", $buffer[0]); // vbgamez STANDARD
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_10(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    if ($server['b']['type'] == "quakewars") { fwrite($vbgamez_fp, "\xFF\xFFgetInfoEX\xFF"); }

    else                                     { fwrite($vbgamez_fp, "\xFF\xFFgetInfo\xFF");   }
...
//---------------------------------------------------------+
    ...
    $buffer = vbgamez_parse_color($buffer, "2");
...
//---------------------------------------------------------+
...

    {
...

      {
        ...
        $server['p'][$player_key]['ping']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

        $server['p'][$player_key]['rate']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

        $server['p'][$player_key]['unknown'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

        $player_name                         = vbgamez_cut_string($buffer);

        $player_tag_position                 = ord(vbgamez_cut_byte($buffer, 1));

        $player_tag                          = vbgamez_cut_string($buffer);

        $server['p'][$player_key]['bot']     = ord(vbgamez_cut_byte($buffer, 1));
...
//---------------------------------------------------------+
...

    {
      ...
      $server['e']['si_osmask']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "I");

      $server['e']['si_ranked']    = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['si_timeleft']  = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "I") / 1000);

      $server['e']['si_gamestate'] = ord(vbgamez_cut_byte($buffer, 1));
...

      {
        ...
        $server['p'][$player_key]['xp']     = intval(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

        $server['p'][$player_key]['team']   = vbgamez_cut_string($buffer);

        $server['p'][$player_key]['score']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "i");

        $server['p'][$player_key]['deaths'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "i");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_11(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  UT3 RESPONSE IS REALLY MESSY SO THIS CLEANS IT UP
    ...
    $status = vbgamez_query_06($server, $vbgamez_need, $vbgamez_fp);
...
//---------------------------------------------------------+
    ...
    foreach ($vbgamez_ut3_key as $old => $new)
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_12(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  VICE CITY CURRENTLY ONLY SUPPORTS THE 'i' CHALLENGE
    ...
    elseif ($server['b']['type'] == "vcmp") { $challenge_packet = "VCMP\x21\x21\x21\x21\x00\x00"; $vbgamez_need['e'] = FALSE; $vbgamez_need['p'] = FALSE; }



    if     ($vbgamez_need['s']) { $challenge_packet .= "i"; }

    elseif ($vbgamez_need['e']) { $challenge_packet .= "r"; }

    elseif ($vbgamez_need['p']) { $challenge_packet .= "d"; }



    fwrite($vbgamez_fp, $challenge_packet);
...
//---------------------------------------------------------+
...

    {
      ...
      $vbgamez_need['s'] = FALSE;



      $server['s']['password']   = ord(vbgamez_cut_byte($buffer, 1));

      $server['s']['players']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

      $server['s']['playersmax'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

      $server['s']['name']       = vbgamez_cut_pascal($buffer, 4);

      $server['e']['gamemode']   = vbgamez_cut_pascal($buffer, 4);

      $server['s']['map']        = vbgamez_cut_pascal($buffer, 4);
...
//---------------------------------------------------------+
...

    {
      ...
      $vbgamez_need['e'] = FALSE;



      $item_total = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

      {
        ...
        $data_key   = strtolower(vbgamez_cut_pascal($buffer));

        $data_value = vbgamez_cut_pascal($buffer);
...
//---------------------------------------------------------+
...

    {
      ...
      $player_total = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

      {
        ...
        $server['p'][$i]['pid']   = ord(vbgamez_cut_byte($buffer, 1));

        $server['p'][$i]['name']  = vbgamez_cut_pascal($buffer);

        $server['p'][$i]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "S");

        $server['p'][$i]['ping']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "S");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_13(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    $buffer_s = ""; fwrite($vbgamez_fp, "\x21\x21\x21\x21\x00"); // REQUEST [s]

    $buffer_e = ""; fwrite($vbgamez_fp, "\x21\x21\x21\x21\x01"); // REQUEST [e]

    $buffer_p = ""; fwrite($vbgamez_fp, "\x21\x21\x21\x21\x02"); // REQUEST [p]
...
//---------------------------------------------------------+
    ...
    while ($packet = fread($vbgamez_fp, 4096))
...
//  CERTAIN CHARACTERS CAUSE A WRONG PASCAL LENGTH AND NULLS TO APPEAR WITHIN NAMES
    ...
    $server['e']['hostport']   = vbgamez_unpack(vbgamez_cut_byte($buffer_s, 4), "S");

    $buffer_s = substr($buffer_s, 4);

    $server['s']['name']       = vbgamez_cut_string($buffer_s, 1);

    $server['s']['map']        = vbgamez_cut_string($buffer_s, 1);

    $server['e']['gamemode']   = vbgamez_cut_string($buffer_s, 1);

    $server['s']['players']    = vbgamez_unpack(vbgamez_cut_byte($buffer_s, 4), "S");

    $server['s']['playersmax'] = vbgamez_unpack(vbgamez_cut_byte($buffer_s, 4), "S");
...
//---------------------------------------------------------+
...

    {
      ...
      $item_key   = strtolower(vbgamez_cut_string($buffer_e, 1));

      $item_value = vbgamez_cut_string($buffer_e, 1);
...
//---------------------------------------------------------+
...

    {
      ...
      $server['p'][$player_key]['pid']   = vbgamez_unpack(vbgamez_cut_byte($buffer_p, 4), "S");
      ...
      $server['p'][$player_key]['name']  = vbgamez_cut_string($buffer_p, 1, $end_marker);

      $server['p'][$player_key]['ping']  = vbgamez_unpack(vbgamez_cut_byte($buffer_p, 4), "S");

      $server['p'][$player_key]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer_p, 4), "s");

      $tmp                               = vbgamez_cut_byte($buffer_p, 4);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_14(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://flstat.cryosphere.co.uk/global-list.php
    ...
    fwrite($vbgamez_fp, "\x00\x02\xf1\x26\x01\x26\xf0\x90\xa6\xf0\x26\x57\x4e\xac\xa0\xec\xf8\x68\xe4\x8d\x21");
...
//---------------------------------------------------------+
    ...
    $server['s']['password']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l") - 1 ? 1 : 0;

    $server['s']['playersmax'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l") - 1;

    $server['s']['players']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l") - 1;

    $buffer                    = substr($buffer, 4);  // UNKNOWN ( 88 )

    $name_length               = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

    $buffer                    = substr($buffer, 56); // UNKNOWN

    $server['s']['name']       = vbgamez_cut_byte($buffer, $name_length);



    vbgamez_cut_string($buffer, 0, ":");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_15(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "GTR2_Direct_IP_Search\x00");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_16(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  $server['e']['gamename']         = vbgamez_get_string($buffer);

    $buffer = substr($buffer, 8);

//  $server['e']['fullupdate']       = vbgamez_unpack($buffer[0], "C");

    $server['e']['region']           = vbgamez_unpack($buffer[1] .$buffer[2],  "S");

//  $server['e']['ip']               = ($buffer[3] .$buffer[4].$buffer[5].$buffer[6]); // UNSIGNED LONG

//  $server['e']['size']             = vbgamez_unpack($buffer[7] .$buffer[8],  "S");

    $server['e']['version']          = vbgamez_unpack($buffer[9] .$buffer[10], "S");

//  $server['e']['version_racecast'] = vbgamez_unpack($buffer[11].$buffer[12], "S");

    $server['e']['hostport']         = vbgamez_unpack($buffer[13].$buffer[14], "S");

//  $server['e']['queryport']        = vbgamez_unpack($buffer[15].$buffer[16], "S");

    $buffer = substr($buffer, 17);

    $server['s']['game']             = vbgamez_get_string($buffer);

    $buffer = substr($buffer, 20);

    $server['s']['name']             = vbgamez_get_string($buffer);

    $buffer = substr($buffer, 28);

    $server['s']['map']              = vbgamez_get_string($buffer);

    $buffer = substr($buffer, 32);

    $server['e']['motd']             = vbgamez_get_string($buffer);

    $buffer = substr($buffer, 96);

    $server['e']['packed_aids']      = vbgamez_unpack($buffer[0].$buffer[1], "S");

//  $server['e']['ping']             = vbgamez_unpack($buffer[2].$buffer[3], "S");

    $server['e']['packed_flags']     = vbgamez_unpack($buffer[4],  "C");

    $server['e']['rate']             = vbgamez_unpack($buffer[5],  "C");

    $server['s']['players']          = vbgamez_unpack($buffer[6],  "C");

    $server['s']['playersmax']       = vbgamez_unpack($buffer[7],  "C");

    $server['e']['bots']             = vbgamez_unpack($buffer[8],  "C");

    $server['e']['packed_special']   = vbgamez_unpack($buffer[9],  "C");

    $server['e']['damage']           = vbgamez_unpack($buffer[10], "C");

    $server['e']['packed_rules']     = vbgamez_unpack($buffer[11].$buffer[12], "S");

    $server['e']['credits1']         = vbgamez_unpack($buffer[13], "C");

    $server['e']['credits2']         = vbgamez_unpack($buffer[14].$buffer[15], "S");

    $server['e']['time']   = vbgamez_time(vbgamez_unpack($buffer[16].$buffer[17], "S"));

    $server['e']['laps']             = vbgamez_unpack($buffer[18].$buffer[19], "s") / 16;

    $buffer = substr($buffer, 23);

    $server['e']['vehicles']         = vbgamez_get_string($buffer);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_17(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://masterserver.savage.s2games.com
    ...
    fwrite($vbgamez_fp, "\x9e\x4c\x23\x00\x00\xce\x21\x21\x21\x21");
...
//---------------------------------------------------------+
    ...
    while ($key = strtolower(vbgamez_cut_string($buffer, 0, "\xFE")))

    {
      ...
      $value = vbgamez_cut_string($buffer, 0, "\xFF");

      $value = str_replace("\x00", "", $value);

      $value = vbgamez_parse_color($value, $server['b']['type']);
...
//---------------------------------------------------------+
    ...
    while ($value = vbgamez_cut_string($buffer, 0, "\x0a"))

    {
      ...
      $server['p'][$player_key]['name'] = vbgamez_parse_color(substr($value, 1), $server['b']['type']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_18(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    $server['e']['time']            = vbgamez_cut_string($buffer);

    $server['s']['map']             = vbgamez_cut_string($buffer);

    $server['e']['nextmap']         = vbgamez_cut_string($buffer);

    $server['e']['location']        = vbgamez_cut_string($buffer);

    $server['e']['minimum_players'] = ord(vbgamez_cut_string($buffer));

    $server['e']['gamemode']        = vbgamez_cut_string($buffer);

    $server['e']['version']         = vbgamez_cut_string($buffer);

    $server['e']['minimum_level']   = ord(vbgamez_cut_byte($buffer, 1));
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_19(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "\xC0\xDE\xF1\x11\x42\x06\x00\xF5\x03\x21\x21\x21\x21");
...
//---------------------------------------------------------+
    ...
    $server['s']['name']       = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['s']['map']        = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['e']['nextmap']    = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['e']['gametype']   = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));
    ...
    $server['s']['playersmax'] = ord(vbgamez_cut_byte($buffer, 4));

    $server['s']['players']    = ord(vbgamez_cut_byte($buffer, 4));
...
//---------------------------------------------------------+
...

    {
     ...
     $server['p'][$player_key]['name'] = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));
...
//---------------------------------------------------------+
    ...
    $server['e']['version']    = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['e']['mods']       = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['e']['dedicated']  = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['time']       = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['status']     = ord(vbgamez_cut_byte($buffer, 4));

    $server['e']['gamemode']   = ord(vbgamez_cut_byte($buffer, 4));

    $server['e']['motd']       = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));

    $server['e']['respawns']   = ord(vbgamez_cut_byte($buffer, 4));

    $server['e']['time_limit'] = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['voting']     = ord(vbgamez_cut_byte($buffer, 4));
...
//---------------------------------------------------------+
...

    {
     ...
     $server['p'][$player_key]['team'] = ord(vbgamez_cut_byte($buffer, 4));



     $unknown = ord(vbgamez_cut_byte($buffer, 1));
...
//---------------------------------------------------------+
    ...
    $server['e']['platoon_1_color']   = ord(vbgamez_cut_byte($buffer, 8));

    $server['e']['platoon_2_color']   = ord(vbgamez_cut_byte($buffer, 8));

    $server['e']['platoon_3_color']   = ord(vbgamez_cut_byte($buffer, 8));

    $server['e']['platoon_4_color']   = ord(vbgamez_cut_byte($buffer, 8));

    $server['e']['timer_on']          = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['timer_time']        = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['time_debriefing']   = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['time_respawn_min']  = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['time_respawn_max']  = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['time_respawn_safe'] = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['difficulty']        = ord(vbgamez_cut_byte($buffer, 4));

    $server['e']['respawn_total']     = ord(vbgamez_cut_byte($buffer, 4));

    $server['e']['random_insertions'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['spectators']        = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['arcademode']        = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['ai_backup']         = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['random_teams']      = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['time_starting']     = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"));

    $server['e']['identify_friends']  = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['identify_threats']  = ord(vbgamez_cut_byte($buffer, 1));
    ...
    $server['e']['restrictions']      = vbgamez_get_string(vbgamez_cut_pascal($buffer, 4, 3, -3));
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_20(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
...

    {
      ...
      fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFFFLSQ");
...

    {
      ...
      fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x57");
      ...
      if     ($vbgamez_need['e']) { fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x56{$challenge_code}"); }
...
//---------------------------------------------------------+
...

    {
      ...
      $server['e']['hostport']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "n");

      $server['s']['players']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['s']['playersmax']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['e']['dedicated']   = vbgamez_cut_byte($buffer, 1);

      $server['e']['os']          = vbgamez_cut_byte($buffer, 1);

      $server['s']['password']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['e']['anticheat']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['e']['cpu_load']    = round(3.03 * vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C"))."%";

      $server['e']['round']       = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['e']['roundsmax']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

      $server['e']['timeleft']    = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") / 250);
...

    {
...

      {
        ...
        $server['p'][$player_key]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "N");

        $server['p'][$player_key]['time']  = vbgamez_time(vbgamez_unpack(strrev(vbgamez_cut_byte($buffer, 4)), "f"));

        $server['p'][$player_key]['ping']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "n");

        $server['p'][$player_key]['uid']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "N");

        $server['p'][$player_key]['team']  = ord(vbgamez_cut_byte($buffer, 1));
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_21(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp,"\xff\xff\xff\xff\xff\xff\xff\xff\xff\xffgief");
...
//---------------------------------------------------------+
    ...
    $server['s']['password']   = vbgamez_cut_string($buffer);

    $server['e']['progress']   = vbgamez_cut_string($buffer)."%";

    $server['s']['players']    = vbgamez_cut_string($buffer);

    $server['s']['playersmax'] = vbgamez_cut_string($buffer);
...
//---------------------------------------------------------+
...

    {
      ...
      $server['p'][$player_key]['score'] = vbgamez_cut_string($buffer);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_22(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    $response_type = ord(vbgamez_cut_byte($buffer, 1)); // TYPE SHOULD BE 4
...
//---------------------------------------------------------+
    ...
    $grf_count = ord(vbgamez_cut_byte($buffer, 1));
...

    {
      ...
      $server['e']['grf_'.$a.'_id'] = strtoupper(dechex(vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "N")));
...

      {
        ...
        $server['e']['grf_'.$a.'_md5'] .= strtoupper(dechex(ord(vbgamez_cut_byte($buffer, 1))));
...
//---------------------------------------------------------+
    ...
    $server['e']['date_current']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "L");

    $server['e']['date_start']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "L");

    $server['e']['companies_max']  = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['companies']      = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['spectators_max'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['s']['name']           = vbgamez_cut_string($buffer);

    $server['e']['version']        = vbgamez_cut_string($buffer);

    $server['e']['language']       = ord(vbgamez_cut_byte($buffer, 1));
    ...
    $server['e']['map_width']      = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

    $server['e']['map_height']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

    $server['e']['map_set']        = ord(vbgamez_cut_byte($buffer, 1));
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_23(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    $server['s']['game']       = vbgamez_cut_pascal($buffer);

    $server['e']['version']    = vbgamez_cut_pascal($buffer);

    $server['s']['name']       = vbgamez_cut_pascal($buffer);
    ...
    $server['e']['cpu']        = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

    $server['e']['mod']        = vbgamez_cut_pascal($buffer);

    $server['e']['type']       = vbgamez_cut_pascal($buffer);

    $server['s']['map']        = vbgamez_cut_pascal($buffer);

    $server['e']['motd']       = vbgamez_cut_pascal($buffer);

    $server['e']['teams']      = ord(vbgamez_cut_byte($buffer, 1));
...
//---------------------------------------------------------+
    ...
    $team_field = "?".vbgamez_cut_pascal($buffer);
...
//---------------------------------------------------------+
    ...
    $player_field = "?".vbgamez_cut_pascal($buffer);
...
//---------------------------------------------------------+
...

    {
      ...
      $team_name = vbgamez_cut_pascal($buffer);

      $team_info = vbgamez_cut_pascal($buffer);
...
//---------------------------------------------------------+
...

    {
      ...
      $player_bits[] = ord(vbgamez_cut_byte($buffer, 1)) * 4; // %p = PING

      $player_bits[] = ord(vbgamez_cut_byte($buffer, 1));     // %l = PACKET LOSS

      $player_bits[] = ord(vbgamez_cut_byte($buffer, 1));     // %t = TEAM

      $player_bits[] = vbgamez_cut_pascal($buffer);           // %n = PLAYER NAME

      $player_info   = vbgamez_cut_pascal($buffer);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_24(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
...

    {
      ...
      $server['e']['gamemode']   = ord(vbgamez_cut_byte($buffer, 1));

      $server['s']['players']    = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['timeleft']   = vbgamez_time(ord(vbgamez_cut_byte($buffer, 1)) * 60);
...

    {
      ...
      $server['e']['version']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

    {
      ...
      $info_returned             = ord(vbgamez_cut_byte($buffer, 1)); // CODED FOR 5
      ...
      $server['s']['password']   = ord(vbgamez_cut_byte($buffer, 1)); // BIT FIELD
...

    {
      ...
      $info_returned             = ord(vbgamez_cut_byte($buffer, 1)); // CODED FOR 6
      ...
      $server['e']['mutators']   = ord(vbgamez_cut_byte($buffer, 1));
...
//  AND THERE CAN BE ID GAPS BETWEEN THE PLAYERS RETURNED
    ...
    if ($vbgamez_need['p'] && $server['s']['players'])

    {
...

      {
        ...
        fwrite($vbgamez_fp, "\x00\x01".chr($player_id));
...

        {
          ...
          $server['p'][$player_key]['pid']       = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['ping']      = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['ping']      = $server['p'][$player_key]['ping'] == 128 ? vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") : $server['p'][$player_key]['ping'];

          $server['p'][$player_key]['name']      = vbgamez_cut_string($buffer);

          $server['p'][$player_key]['team']      = vbgamez_cut_string($buffer);

          $server['p'][$player_key]['score']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "c");

          $server['p'][$player_key]['damage']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['deaths']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['teamkills'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['accuracy']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C")."%";

          $server['p'][$player_key]['health']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "c");

          $server['p'][$player_key]['spree']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");

          $server['p'][$player_key]['weapon']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");
...

        {
          ...
          $server['p'][$player_key]['armour']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 1), "C");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_25(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://www.tribesnext.com
    ...
    fwrite($vbgamez_fp,"\x12\x02\x21\x21\x21\x21");
...
//---------------------------------------------------------+
    ...
    $server['e']['gamemode']   = vbgamez_cut_pascal($buffer);

    $server['s']['map']        = vbgamez_cut_pascal($buffer);

    $server['e']['bit_flags']  = ord(vbgamez_cut_byte($buffer, 1));
    ...
    $server['e']['unknown']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...
//---------------------------------------------------------+
    ...
    $team_total = vbgamez_cut_string($buffer, 0, "\x0A");
...

    {
      ...
      $server['t'][$i]['name']  = vbgamez_cut_string($buffer, 0, "\x09");

      $server['t'][$i]['score'] = vbgamez_cut_string($buffer, 0, "\x0A");

    }



    $player_total = vbgamez_cut_string($buffer, 0, "\x0A");
...

    {
      ...
      vbgamez_cut_byte($buffer, 1); // ? 16

      vbgamez_cut_byte($buffer, 1); // ? 8 or 14 = BOT / 12 = ALIAS / 11 = NORMAL

      if (ord($buffer[0]) < 32) { vbgamez_cut_byte($buffer, 1); } // ? 8 PREFIXES SOME NAMES



      $server['p'][$i]['name']  = vbgamez_cut_string($buffer, 0, "\x11");

                                  vbgamez_cut_string($buffer, 0, "\x09"); // ALWAYS BLANK

      $server['p'][$i]['team']  = vbgamez_cut_string($buffer, 0, "\x09");

      $server['p'][$i]['score'] = vbgamez_cut_string($buffer, 0, "\x0A");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_26(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  http://aluigi.altervista.org/papers.htm#aa3authdec
    ...
    vbgamez_gs_crypt($server['b']['type'], $packet, TRUE);

    fwrite($vbgamez_fp, "\x4A\x35\xFF\xFF\x02\x00\x02\x00\x01\x00{$packet}");
...
//---------------------------------------------------------+
    ...
    vbgamez_gs_crypt($server['b']['type'], $buffer, FALSE);
...
//---------------------------------------------------------+
...

    {
      ...
      $raw_name = vbgamez_cut_pascal($buffer, 2);

      $raw_type = vbgamez_cut_byte($buffer, 1);
...

      {
        ...
        $raw[$raw_name] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "i");
        ...
        $raw_total = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

        {
          ...
          $raw_value = vbgamez_cut_pascal($buffer, 2);
...
//---------------------------------------------------------+
...

    {
...

      {
        ...
        // vbgamez STANDARD ( SWAP NAME AS ITS ACTUALLY THE ACCOUNT NAME )
    ...
    $vbgamez_conversion = array("gamename"=>"name","mapname"=>"map","playercount"=>"players","maxplayers"=>"playersmax","flagpassword"=>"password");

    foreach ($vbgamez_conversion as $e => $s) { $server['s'][$s] = $server['e'][$e]; unset($server['ea'][$e]); } // vbgamez STANDARD
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_27(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  http://www.greycube.com/help/vbgamez_other/skulltag_huffman.txt
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "\x02\xB8\x49\x1A\x9C\x8B\xB5\x3F\x1E\x8F\x07");
...
//---------------------------------------------------------+
    ...
    $response_status        = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l"); if ($response_status != "5660023") { return FALSE; }

    $response_time          = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

    $server['e']['version'] = vbgamez_cut_string($buffer);

    $response_flag          = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");
...
//---------------------------------------------------------+
    ...
    if ($response_flag & 0x00000001) { $server['s']['name']       = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000002) { $server['e']['wadurl']     = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000004) { $server['e']['email']      = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000008) { $server['s']['map']        = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000010) { $server['s']['playersmax'] = ord(vbgamez_cut_byte($buffer, 1)); }

    if ($response_flag & 0x00000020) { $server['e']['playersmax'] = ord(vbgamez_cut_byte($buffer, 1)); }

    if ($response_flag & 0x00000040)

    {

      $pwad_total = ord(vbgamez_cut_byte($buffer, 1));
      ...
      for ($i=0; $i<$pwad_total; $i++)
        ...
        $server['e']['pwads'] .= vbgamez_cut_string($buffer)." ";
...

    {
      ...
      $server['e']['gametype'] = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['instagib'] = ord(vbgamez_cut_byte($buffer, 1));

      $server['e']['buckshot'] = ord(vbgamez_cut_byte($buffer, 1));

    }

    if ($response_flag & 0x00000100) { $server['s']['game']         = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000200) { $server['e']['iwad']         = vbgamez_cut_string($buffer); }

    if ($response_flag & 0x00000400) { $server['s']['password']     = ord(vbgamez_cut_byte($buffer, 1)); }

    if ($response_flag & 0x00000800) { $server['e']['playpassword'] = ord(vbgamez_cut_byte($buffer, 1)); }

    if ($response_flag & 0x00001000) { $server['e']['skill']        = ord(vbgamez_cut_byte($buffer, 1)) + 1; }

    if ($response_flag & 0x00002000) { $server['e']['botskill']     = ord(vbgamez_cut_byte($buffer, 1)) + 1; }

    if ($response_flag & 0x00004000)

    {

      $server['e']['dmflags']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

      $server['e']['dmflags2']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

      $server['e']['compatflags'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");
...

    {
      ...
      $server['e']['fraglimit'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $timelimit                = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...

      {
        ...
        $server['e']['timeleft'] = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") * 60);

      }



      $server['e']['timelimit']  = vbgamez_time($timelimit * 60);

      $server['e']['duellimit']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $server['e']['pointlimit'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $server['e']['winlimit']   = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

    }

    if ($response_flag & 0x00020000) { $server['e']['teamdamage'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f"); }

    if ($response_flag & 0x00040000) // DEPRECIATED

    {

      $server['t'][0]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $server['t'][1]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

    }

    if ($response_flag & 0x00080000) { $server['s']['players'] = ord(vbgamez_cut_byte($buffer, 1)); }
...

    {
...

      {
        ...
        $server['p'][$i]['name']      = vbgamez_parse_color(vbgamez_cut_string($buffer), $server['b']['type']);

        $server['p'][$i]['score']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

        $server['p'][$i]['ping']      = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

        $server['p'][$i]['spectator'] = ord(vbgamez_cut_byte($buffer, 1));

        $server['p'][$i]['bot']       = ord(vbgamez_cut_byte($buffer, 1));
...

        {
          ...
          $server['p'][$i]['team'] = ord(vbgamez_cut_byte($buffer, 1));

        }



        $server['p'][$i]['time'] = vbgamez_time(ord(vbgamez_cut_byte($buffer, 1)) * 60);
...

    {
      ...
      $team_total = ord(vbgamez_cut_byte($buffer, 1));
...

      {
        ...
        for ($i=0; $i<$team_total; $i++) { $server['t'][$i]['name'] = vbgamez_cut_string($buffer); }
...

      {
        ...
        for ($i=0; $i<$team_total; $i++) { $server['t'][$i]['color'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l"); }
...

      {
        ...
        for ($i=0; $i<$team_total; $i++) { $server['t'][$i]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s"); }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_28(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://doomutils.ucoz.com
    ...
    fwrite($vbgamez_fp, "\xA3\xDB\x0B\x00"."\xFC\xFD\xFE\xFF"."\x01\x00\x00\x00"."\x21\x21\x21\x21");
...
//---------------------------------------------------------+
    ...
    $response_status  = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l"); if ($response_status != "5560022") { return FALSE; }

    $response_version = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

    $response_time    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");



    $server['e']['invited']    = ord(vbgamez_cut_byte($buffer, 1));
...

    {
      ...
      $pwad_optional         = ord(vbgamez_cut_byte($buffer, 1));

      $pwad_alternative      = vbgamez_cut_string($buffer);
    ...
    $server['e']['iwad']       = vbgamez_cut_string($buffer);

    $iwad_altenative           = vbgamez_cut_string($buffer);

    $server['e']['skill']      = ord(vbgamez_cut_byte($buffer, 1)) + 1;

    $server['e']['wadurl']     = vbgamez_cut_string($buffer);

    $server['e']['email']      = vbgamez_cut_string($buffer);
    ...
    $server['e']['inviteonly'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['players']    = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['playersmax'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['timelimit']  = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") * 60);

    $server['e']['timeleft']   = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") * 60);

    $server['e']['fraglimit']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

    $server['e']['gravity']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f");

    $server['e']['aircontrol'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f");

    $server['e']['playersmin'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['removebots'] = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['voting']     = ord(vbgamez_cut_byte($buffer, 1));

    $server['e']['serverinfo'] = vbgamez_cut_string($buffer);

    $server['e']['startup']    = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");
...

    {
      ...
      $server['p'][$i]['name']      = vbgamez_cut_string($buffer);

      $server['p'][$i]['score']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $server['p'][$i]['death']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");

      $server['p'][$i]['ping']      = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");

      $server['p'][$i]['time']      = vbgamez_time(vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S") * 60);
      ...
      $server['p'][$i]['country']   = vbgamez_cut_byte($buffer, 2);
    ...
    $server['e']['teamdamage'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "f");
...

    {
      ...
      $server['t']['team'][$i]['name']  = vbgamez_cut_string($buffer);

      $server['t']['team'][$i]['color'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

      $server['t']['team'][$i]['score'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "s");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_29(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  REFERENCE: http://www.cs2d.com/servers.php
...

    {
      ...
      fwrite($vbgamez_fp, "\x01\x00\x03\x10\x21\xFB\x01\x75\x00");
      ...
      $server['e']['bit_flags']  = ord(vbgamez_cut_byte($buffer, 1)) - 48;
...

    {
      ...
      fwrite($vbgamez_fp, "\x01\x00\xFB\x05");
      ...
      $player_total = ord(vbgamez_cut_byte($buffer, 1));
...

      {
        ...
        $server['p'][$i]['score']  = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");

        $server['p'][$i]['deaths'] = vbgamez_unpack(vbgamez_cut_byte($buffer, 4), "l");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_30(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  THIS USES TCP COMMUNICATION
...

    {
      ...
      fwrite($vbgamez_fp, "\x00\x00\x00\x00\x1B\x00\x00\x00\x01\x00\x00\x00\x0A\x00\x00\x00serverInfo\x00");
...

    {
      ...
      fwrite($vbgamez_fp, "\x00\x00\x00\x00\x24\x00\x00\x00\x02\x00\x00\x00\x0B\x00\x00\x00listPlayers\x00\x03\x00\x00\x00all\x00");
...
//---------------------------------------------------------+
    ...
    $length = vbgamez_unpack(substr($buffer, 4, 4), "L");
...
//---------------------------------------------------------+
    ...
    $response_type = vbgamez_cut_pascal($buffer, 4, 0, 1);
...
//---------------------------------------------------------+
...

    {
      ...
      $server['s']['name']            = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['s']['players']         = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['s']['playersmax']      = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['e']['gamemode']        = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['s']['map']             = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['e']['score_attackers'] = vbgamez_cut_pascal($buffer, 4, 0, 1);

      $server['e']['score_defenders'] = vbgamez_cut_pascal($buffer, 4, 0, 1);
...
//---------------------------------------------------------+
...

    {
      ...
      $field_total = vbgamez_cut_pascal($buffer, 4, 0, 1);
...

      {
        ...
        $field_list[] = strtolower(vbgamez_cut_pascal($buffer, 4, 0, 1));
      ...
      $player_total = vbgamez_cut_pascal($buffer, 4, 0, 1);
...

      {
...

        {
          ...
          $value = vbgamez_cut_pascal($buffer, 4, 0, 1);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_31(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//  SERVER RULES ARE ON THE END OF THE INFO RESPONSE
    ...
    fwrite($vbgamez_fp, "\xFF\xFF\xFF\xFF\x54Source Engine Query\x00");
    ...
    $server['e']['hostport']     = vbgamez_unpack(vbgamez_cut_byte($buffer, 2), "S");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_32(&$server, &$vbgamez_need, &$vbgamez_fp)
...
//---------------------------------------------------------+
    ...
    fwrite($vbgamez_fp, "\x05\x00\x00\x01\x0A");
...
function vbgamez_query_97(&$server, &$vbgamez_need, &$vbgamez_fp)

{

if (strpos(fread($vbgamez_fp, 4096), 'TS') === FALSE) { return FALSE; }
...
if ($ver) { fread($vbgamez_fp, 4096); }

fwrite($vbgamez_fp, $param[$ver][0].$server['b']['c_port']."\n");	// select virtualserver

if (strtoupper(substr(fread($vbgamez_fp, 4096), -4, -2)) != 'OK') { return FALSE; }



fwrite($vbgamez_fp, $param[$ver][1]."\n");	// request serverinfo

$buffer = fread($vbgamez_fp, 4096);

if (!$buffer || substr($buffer, 0, 5) == 'error') { return FALSE; }

while (strtoupper(substr($buffer, -4, -2)) != 'OK') { $part = fread($vbgamez_fp, 4096);
...
while ($val = vbgamez_cut_string($buffer, 7+7*$ver, $param[$ver][2])) {

$key = vbgamez_cut_string($val, 0, '='); $items[$key] = $val;

}

if (!isset($items['name'])) { return FALSE; }

$server['s']['name']	= $ver ? vbgamez_unescape($items['name']) : $items['name'];
...
$server['e']['motd']	= $ver ? vbgamez_unescape($items['welcomemessage']) : $items['welcomemessage'];

$server['e']['uptime']	= vbgamez_time($items['uptime']);

$server['e']['channels']	= $items[$ver ? 'channelsonline' : 'currentchannels'];

if ($ver) { $server['e']['version'] = vbgamez_unescape($items['version']); }

if (!$vbgamez_need['p'] || $server['s']['players'] < 1) { return TRUE; }



fwrite($vbgamez_fp, $param[$ver][3]."\n");	// request playerlist

$buffer = fread($vbgamez_fp, 4096);

while (substr($buffer, -4) != "OK\r\n" && substr($buffer, -2) != "\n\r") { $part = fread($vbgamez_fp, 4096);
...
while ($items = vbgamez_cut_string($buffer, 0, '|')) {

vbgamez_cut_string($items, 0, 'e='); $name = vbgamez_cut_string($items, 0, ' ');

if (substr($name, 0, 15) == 'Unknown\sfrom\s') { continue; }

$server['p'][$i]['name'] = vbgamez_unescape($name); vbgamez_cut_string($items, 0, 'ry');
...
$server['p'][$i]['time'] = vbgamez_time($items[8]); $i++;
...
function vbgamez_unescape($text) {
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_query_feed(&$server, $request, $vbgamez_feed_method, $vbgamez_feed_url)
...
//---------------------------------------------------------+
    ...
    $vbgamez_feed_error = 0;



    $host = parse_url($vbgamez_feed_url);



    if (empty($host['host']) || empty($host['path'])) { vB_vBGamez::error("vbgamez FEED PROBLEM: INVALID URL"); }
...
//---------------------------------------------------------+
    ...
    if (function_exists('curl_init') && function_exists('curl_setopt') && function_exists('curl_exec') && $vbgamez_feed_method == 1)

    {

      $vbgamez_curl = curl_init();



      curl_setopt($vbgamez_curl, CURLOPT_HEADER, 0);

      curl_setopt($vbgamez_curl, CURLOPT_HTTPGET, 1);

      curl_setopt($vbgamez_curl, CURLOPT_TIMEOUT, 6);

      curl_setopt($vbgamez_curl, CURLOPT_ENCODING, "");

      curl_setopt($vbgamez_curl, CURLOPT_FORBID_REUSE, 1);

      curl_setopt($vbgamez_curl, CURLOPT_FRESH_CONNECT, 1);

      curl_setopt($vbgamez_curl, CURLOPT_RETURNTRANSFER, 1);

      curl_setopt($vbgamez_curl, CURLOPT_CONNECTTIMEOUT, 6);

      curl_setopt($vbgamez_curl, CURLOPT_REFERER, $referrer);

      curl_setopt($vbgamez_curl, CURLOPT_URL, "http://{$host['host']}{$host['path']}{$host_query}");



      $http_reply = curl_exec($vbgamez_curl);



      if (curl_error($vbgamez_curl))

      {

        $vbgamez_feed_error = 1;

      }



      curl_close($vbgamez_curl);
...
//---------------------------------------------------------+
...

    {
      ...
      $vbgamez_fp = @fsockopen($host['host'], "80", $errno, $errstr, 6);
...

      {
        ...
        stream_set_timeout($vbgamez_fp, 6, 0);
        ...
        while (!feof($vbgamez_fp))

        {

          $http_chunk = fread($vbgamez_fp, 4096);
...
//---------------------------------------------------------+
...

    {
      ...
      vB_vBGamez::error("vbgamez FEED PROBLEM: NO CURL OR FSOCKOPEN SUPPORT");
...
//---------------------------------------------------------+
...

    {
      ...
      elseif (preg_match("/_SvbgamezF_(.*)_SvbgamezF_/U", $http_reply, $match))
...

      {
        ...
        $vbgamez_feed_error = 2;

      }



      if (!$vbgamez_feed_error && empty($server))

      {

        $vbgamez_feed_error = 3;
...
//---------------------------------------------------------+
...

    {
      ...
      case 2: // NO FEED DATA - MAYBE WRONG FEED URL
        ...
        vB_vBGamez::error("FEED MISSING FROM: {$host['host']}{$host['path']} RETURNED: ".htmlspecialchars($http_reply, ENT_QUOTES)." :END");
      ...
      case 3: // UNABLE TO UNPACK FEED DATA - MAYBE ERRORS ON FEED
        ...
        vB_vBGamez::error("FEED CORRUPTION FROM: {$host['host']}{$host['path']} RETURNED: ".htmlspecialchars($http_reply, ENT_QUOTES)." :END");
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function vbgamez_parse_color($string, $type)
...
//---------------------------------------------------------+
  ...
  function vbgamez_unpack($string, $format)
...
//---------------------------------------------------------+
  ...
  function vbgamez_cut_byte(&$buffer, $length)
...
//---------------------------------------------------------+
  ...
  function vbgamez_cut_string(&$buffer, $start_byte = 0, $end_marker = "\x00")
...
//---------------------------------------------------------+
  ...
  function vbgamez_cut_pascal(&$buffer, $start_byte = 1, $length_adjust = 0, $end_byte = 0)
...
//---------------------------------------------------------+
  ...
  function vbgamez_get_string($buffer, $start_byte = 0, $end_marker = "\x00")
...
//---------------------------------------------------------+
  ...
  function vbgamez_gs_crypt($type, &$buffer, $encrypt = FALSE)

  {
    ...
    if ($type == "aarmy3") { $game_key = "c6mw4it2kg7sz5o0813d9qyufenhj\x00"; } else { vB_vBGamez::error("UNKNOWN GAME CRYPT KEY"); }
